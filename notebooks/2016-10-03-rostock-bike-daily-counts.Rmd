...
daily <- left_join(daily.weather, holidays, by = 'date') %>%
  mutate(holiday = is.na(Feiertage)) %>%
  mutate(holiday = !holiday)

# train test split
train.daily <- daily %>% filter(year(zeitpunkt)<2015)
test.daily <- daily %>% filter(year(zeitpunkt)>=2015)

```

In the last line, I have introduced a categorial variable `rainy_day`, which encodes whether there was precipitation on each given day or not. The tibble `daily.weather` (as far as I am concerned, a tibble is a dataframe with enhanced printing capabilities) now contains daily bicycle counts, temperature, sunshine time and precipitation.
...
ggplot(daily.weather, aes(x=rainy_day, y=n)) + 
...
model.daily.temperature.wday.sunshine.rainy_day <- lm(n~ temperature + weekday + sunshinetime + rainy_day, data = daily.weather)

daily.weather <- daily.weather %>%
  add_residuals(model.daily.temperature.wday.sunshine.rainy_day, var = "temperature.wday.sunshine.rainy_day.detrended")

rms.daily.temp.weekday.sunshine.rainy_day <- sqrt(mean(daily.weather$temperature.wday.sunshine.rainy_day.detrended^2,na.rm = TRUE))

ggplot(daily.weather, aes(date, temperature.wday.sunshine.rainy_day.detrended)) +
  geom_line(alpha=0.2) +
  geom_point(aes(alpha=0.5, size = 2, color=weekday)) +
  geom_ref_line(h=0) +
...
model.daily.temperature.wday.sunshine.rainy_day.holiday <- lm(n~ temperature + weekday + sunshinetime + rainy_day + holiday, data = daily.weather)
...
rms.daily.temp.weekday.sunshine.rainy_day.holiday <- sqrt(mean(daily.weather$temperature.wday.sunshine.rainy_day.holiday.detrended^2,na.rm = TRUE))

ggplot(daily.weather, aes(date, temperature.wday.sunshine.rainy_day.holiday.detrended)) +
  geom_line(alpha=0.2) +
  geom_point(aes(alpha=0.5, size = 2, color=weekday)) +
  geom_ref_line(h=0) +
